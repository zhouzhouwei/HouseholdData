...
for i=1:5
    for j=1:size(Data.PQ,2)/6
        ...
        Varname = ['Household_',num2str((i-1)*30+j),'_PQ5mins'];
        com_var = [Varname,'=PQmin_threedays;'];
        eval(com_var);
        filename=strcat('Household_',num2str((i-1)*30+j),'_PQ5mins.mat');
...
for i=8:12
    for j=1:size(Data.PQ,2)/6
        ...
        Varname = ['Household_',num2str((i-3)*30+j),'_PQ5mins'];
        com_var = [Varname,'=PQmin_threedays;'];
        eval(com_var);
        filename=strcat('Household_',num2str((i-3)*30+j),'_PQ5mins.mat');
        save(filename,Varname);
    end
end
